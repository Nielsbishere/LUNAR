...
private:
	...
	Grad grad3[12];
	Grad grad4[32];
	unsigned char p[512], perm[512], permMod12[512];
	//This is a *lot* faster than floor
...
public:
	SimplexNoise() :
		grad3 {
		Grad(1, 1, 0), Grad(-1, 1, 0), Grad(1, -1, 0), Grad(-1,
			-1,
			0), Grad(1, 0, 1), Grad(-1, 0, 1), Grad(1, 0, -1), Grad(-1,
			0,
			-1), Grad(0, 1, 1), Grad(0, -1, 1), Grad(0, 1, -1), Grad(
				0,
			-1,
			-1)
	}, grad4 {
	...
	double noise(double x, double scale, int octaves, double persistance, double lacunarity) {
		double freq = 1, amp = 1;
		double y = 0;
		for (int i = 0; i < octaves; i++) {
			y += (noise(x / scale * freq) + 1) / 2 * amp;
			amp *= persistance;
			freq *= lacunarity;
		}
		return y / octaves;
	...
	double noise(double x, double y, double scale, int octaves, double persistance, double lacunarity) {
		double freq = 1, amp = 1;
		double z = 0;
		for (int i = 0; i < octaves; i++) {
			z += (noise(x / scale * freq, y / scale * freq) + 1) / 2 * amp;
			amp *= persistance;
			freq *= lacunarity;
		}
		return z;
	...
	double tnoise(double x, double y, double scale, int octaves, double persistance, double lacunarity) {
		double freq = 1, amp = 1;
		double z = 0;
		for (int i = 0; i < octaves; i++) {
			z += (noise(x / scale * freq, y * freq) + 1) / 2 * amp;
			amp *= persistance;
			freq *= lacunarity;
		}
		return z;
	...
	double noise(double x, double y, double z, double scale, int octaves, double persistance, double lacunarity) {
		double freq = 1, amp = 1;
		double w = 0;
		for (int i = 0; i < octaves; i++) {
			w += (noise(x / scale * freq, y / scale * freq, z / scale * freq)
				+ 1) / 2 * amp;
			amp *= persistance;
			freq *= lacunarity;
		}
	...
	double tnoise(double x, double y, double z, double scale, int octaves, double persistance, double lacunarity) {
		double freq = 1, amp = 1;
		double w = 0;
		for (int i = 0; i < octaves; i++) {
			w += (noise(x / scale * freq, y / scale * freq, z * freq)
				+ 1) / 2 * amp;
			amp *= persistance;
			freq *= lacunarity;
		}
	...
	double noise(double x, double y, double z, double a, double scale, int octaves, double persistance, double lacunarity) {
		double freq = 1, amp = 1;
		double w = 0;
		for (int i = 0; i < octaves; i++) {
			w += (noise(x / scale * freq, y / scale * freq, z / scale * freq, a / scale * freq)
				+ 1) / 2 * amp;
			amp *= persistance;
			freq *= lacunarity;
		}
	...
	double tnoise(double x, double y, double z, double a, double scale, int octaves, double persistance, double lacunarity) {
		double freq = 1, amp = 1;
		double w = 0;
		for (int i = 0; i < octaves; i++) {
			w += (noise(x / scale * freq, y / scale * freq, z / scale * freq, a * freq) + 1) / 2 * amp;
			amp *= persistance;
			freq *= lacunarity;
		}
		return w / octaves;
